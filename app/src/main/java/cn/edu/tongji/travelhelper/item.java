package cn.edu.tongji.travelhelper;


import java.io.Serializable;

public class item implements Serializable{
    private String qf;
    private String qf_city;
    private String qf_citycode;
    private String dd;
    private String dd_city;
    private String dd_citycode;
    private String qf_simple;
    private String dd_simple;
    private String qfTerminal;
    private String ddTerminal;
    private String jhqftime;
    private String jhqftime_full;
    private String sjqftime;
    private String sjqftime_full;
    private String jhddtime;
    private String jhddtime_full;
    private String sjddtime;
    private String sjddtime_full;
    private String yjqftime;
    private String yjqftime_full;
    private String yjddtime;
    private String yjddtime_full;
    private String state;
    private String stateid;
    private String djk;
    private String zjgt;
    private String xlzp;
    private String ddzk;

    public String getQf() {
        return qf;
    }

    public void setQf(String qf) {
        this.qf = qf;
    }

    public String getQf_city() {
        return qf_city;
    }

    public void setQf_city(String qf_city) {
        this.qf_city = qf_city;
    }

    public String getQf_citycode() {
        return qf_citycode;
    }

    public void setQf_citycode(String qf_citycode) {
        this.qf_citycode = qf_citycode;
    }

    public String getDd() {
        return dd;
    }

    public void setDd(String dd) {
        this.dd = dd;
    }

    public String getDd_city() {
        return dd_city;
    }

    public void setDd_city(String dd_city) {
        this.dd_city = dd_city;
    }

    public String getDd_citycode() {
        return dd_citycode;
    }

    public void setDd_citycode(String dd_citycode) {
        this.dd_citycode = dd_citycode;
    }

    public String getQf_simple() {
        return qf_simple;
    }

    public void setQf_simple(String qf_simple) {
        this.qf_simple = qf_simple;
    }

    public String getDd_simple() {
        return dd_simple;
    }

    public void setDd_simple(String dd_simple) {
        this.dd_simple = dd_simple;
    }

    public String getQfTerminal() {
        return qfTerminal;
    }

    public void setQfTerminal(String qfTerminal) {
        this.qfTerminal = qfTerminal;
    }

    public String getDdTerminal() {
        return ddTerminal;
    }

    public void setDdTerminal(String ddTerminal) {
        this.ddTerminal = ddTerminal;
    }

    public String getJhqftime() {
        return jhqftime;
    }

    public void setJhqftime(String jhqftime) {
        this.jhqftime = jhqftime;
    }

    public String getJhqftime_full() {
        return jhqftime_full;
    }

    public void setJhqftime_full(String jhqftime_full) {
        this.jhqftime_full = jhqftime_full;
    }

    public String getSjqftime() {
        return sjqftime;
    }

    public void setSjqftime(String sjqftime) {
        this.sjqftime = sjqftime;
    }

    public String getSjqftime_full() {
        return sjqftime_full;
    }

    public void setSjqftime_full(String sjqftime_full) {
        this.sjqftime_full = sjqftime_full;
    }

    public String getJhddtime() {
        return jhddtime;
    }

    public void setJhddtime(String jhddtime) {
        this.jhddtime = jhddtime;
    }

    public String getJhddtime_full() {
        return jhddtime_full;
    }

    public void setJhddtime_full(String jhddtime_full) {
        this.jhddtime_full = jhddtime_full;
    }

    public String getSjddtime() {
        return sjddtime;
    }

    public void setSjddtime(String sjddtime) {
        this.sjddtime = sjddtime;
    }

    public String getSjddtime_full() {
        return sjddtime_full;
    }

    public void setSjddtime_full(String sjddtime_full) {
        this.sjddtime_full = sjddtime_full;
    }

    public String getYjqftime() {
        return yjqftime;
    }

    public void setYjqftime(String yjqftime) {
        this.yjqftime = yjqftime;
    }

    public String getYjqftime_full() {
        return yjqftime_full;
    }

    public void setYjqftime_full(String yjqftime_full) {
        this.yjqftime_full = yjqftime_full;
    }

    public String getYjddtime() {
        return yjddtime;
    }

    public void setYjddtime(String yjddtime) {
        this.yjddtime = yjddtime;
    }

    public String getYjddtime_full() {
        return yjddtime_full;
    }

    public void setYjddtime_full(String yjddtime_full) {
        this.yjddtime_full = yjddtime_full;
    }

    public String getState() {
        return state;
    }

    public void setState(String state) {
        this.state = state;
    }

    public String getStateid() {
        return stateid;
    }

    public void setStateid(String stateid) {
        this.stateid = stateid;
    }

    public String getDjk() {
        return djk;
    }

    public void setDjk(String djk) {
        this.djk = djk;
    }

    public String getZjgt() {
        return zjgt;
    }

    public void setZjgt(String zjgt) {
        this.zjgt = zjgt;
    }

    public String getXlzp() {
        return xlzp;
    }

    public void setXlzp(String xlzp) {
        this.xlzp = xlzp;
    }

    public String getDdzk() {
        return ddzk;
    }

    public void setDdzk(String ddzk) {
        this.ddzk = ddzk;
    }

    public item(){

    }
}
